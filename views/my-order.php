        ...
        <div class="card-body">
            ...
            <div >
                <a href="index.php" class="btn btn-custom" data-abc="true"> <i class="fa fa-chevron-left"></i> Trở lại</a>
                   <?php if($status ==1 ){?>
                    <a href="/DUAN_01/huy-don-hang&id=<?=$order_id?>" class="btn btn-custom" data-abc="true" onclick="return confirmAndReload('Bạn có muốn hủy đơn hàng hay không ?')">  Hủy Đơn Hàng</a>
                    <?php }?>
                <div class="float-right">
                    <span class="text-dark">Thành tiền: </span> 
                    <span style="font-weight: 600;" class="text-danger mr-3"><?=number_format($total)?>₫</span>
                    <a href="/DUAN_01/chi-tiet-don-hang&id=<?=$order_id?>" class="btn btn-custom"> Xem chi tiết</a>
                </div>
            </div>
        </div>
    </article>
...
<script>
    function confirmAndReload(message) {
        if (confirm(message)) {
            window.location.reload();
            return true; // Tiếp tục chuyển hướng khi người dùng đồng ý
        }
        return false; // Ngăn chặn chuyển hướng nếu người dùng không đồng ý
    }
</script>